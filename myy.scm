...
(define primops
   '(+ - * / bit-and bit-or bit-xor eq? cons car cdr if %ref %close %proc emit))
...
(define (replace-first exp node val)
   (if (eq? (car exp) node)
      (cons val (cdr exp))
      (cons (car exp)
         (replace-first (cdr exp) node val))))

(define (literal-ref? x)
   ;; some reference generated by this compiler pass
   (and (pair? x)
      (has? '(%close %proc %ref) (car x))))

(define (maybe-lift-calls exp)
   (let ((call (first literal-ref? exp #false)))
      (if call
         (let ((name (gensym exp)))
            (print "lifting " call " to " name)
            (list
               (list 'lambda (list name)
                  (maybe-lift-calls 
                     (replace-first exp call name)))
               call))
         exp)))

(check '((lambda (G1) (+ a G1)) (%ref L 1))
   (maybe-lift-calls '(+ a (%ref L 1))))

;; convert literal values to references to them via current function and 
;; when the referred value requires a closure, output an expression to generate it 

(define (refer-literals-and-close exp lvar lits op)
   (cond
      ((symbol? exp)
         exp)
      ((null? exp) exp)
      ((quote? exp) 
         (let ((pos (offset lits (cadr exp))))
            (if pos
               ...
               (error "refer-literals-and-close: cannot find " exp))))
      ((offset lits exp) =>
         (λ (pos)
      ...
      ((list? exp)
         ;; (%ref a n) and (%close a 4 (x y z)) are not ANF, so they need to be lifted here
         (maybe-lift-calls
            (map 
               (λ (x) (refer-literals-and-close x lvar lits op))
                exp)))
      (else exp)))
...
(define (refer-env exp lvar env op)
   (cond
      ...
      ((list? exp)
         (maybe-lift-calls
            (map (λ (x) (refer-env x lvar env op)) exp)))
      (else exp)))        
               ...
               ((null? free)
                  ...
                  (ilist '%proc
                     (list 'lambda (cons lvar formals)
                        (refer-literals-and-close body lvar lits '%ref))
                     (map closurize lits)))
               ((null? lits)
               ...
               (else
                  ...
                  (ilist '%clos
                     (ilist '%proc 
                        (list 'lambda (cons lvar formals) 
                           (refer-env 
                              (refer-literals-and-close body lvar lits '%ref2)
                              lvar free '%ref))
                        (map closurize lits))
                     free)))))
      ((quote? exp)
...
(check
   '(%clos (lambda (L x) ((lambda (G1) (+ x G1)) (%ref L 2))) a)
